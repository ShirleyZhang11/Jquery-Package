...
//动画
$.fn.extend({
	 animate: function(target, fn) {
            this.each(function() {
                clearInterval(this.interval);
                //每个元素增加一个定时器,不再公用一个定时器
                this.interval = setInterval(function() {
                    ...
                    for (key in target) {
                        ...
                        var val = 0;
                        if (isOpacity) {
                            val = Math.round(parseFloat($(this).css(key)) * 100);
                        } else {
                            val = parseInt($(this).css(key));
                        }
                        //如果有一个属性不等于最终值,就不应该停止定时器
                        if (val != target[key]) {
                            flag = false;
                        }
                        //速度系数
                        var speed = (target[key] - val) / 9;

                        //根据正负取整,防止循环
                        speed = speed > 0 ? Math.ceil(speed) : Math.floor(speed);

                        //是否添加 px 后缀
                        var suffixes = isOpacity ? '' : 'px';

                        //如果是透明度需要除以100
                        var r = isOpacity ? (val + speed) / 100 : (val + speed);

                        this.style[key] = r + suffixes;
                    }
         ...
         animateWithType: function(target, type, fn) {

             //帧数组函数
             function createTween(start, end, type) {
                 //动画类型
                 this.tweenTypes = {
             ...
             this.each(function() {
                 ...
                 for (key in target) {
                   ...
                   //计算每个元素的每个要动画的属性生成帧数组
                   this.tweens[key] = createTween(val, target[key], type);
                  }
               ...
               this.interval = setInterval(function() {
                   ...
                   for (key in target) {
                       ...
                       var suffixes = isOpacity ? '' : 'px';
                       //如果是透明度需要除以100
                       var r = isOpacity ? (this.tweens[key][this[key].i]) / 100 : (this.tweens[key][this[key].i]);
                       this.style[key] = r + suffixes;
                       this[key].i = this[key].i + 1;

                       if (this[key].i != this.tweens[key].length) {
                           flag = false;
                       }
                   }
                   if (flag) {
